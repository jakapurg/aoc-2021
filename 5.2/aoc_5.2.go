...
func printMatrix(A *mat.Dense){
	fmt.Printf("%v\n\n", mat.Formatted(A, mat.Prefix(" "), mat.Excerpt(10)))
}
func main(){
	...
	for _, val := range text {
		...
		if(beginX==endX || beginY==endY ){
			...
			if(math.Abs(float64(beginX-endX))==math.Abs(float64(beginY-endY))){
				
				tmpVal:=lines.At(beginY,beginX)
				lines.Set(beginY,beginX,tmpVal+1)
